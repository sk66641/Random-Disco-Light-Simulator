document.addEventListener("DOMContentLoaded", () => {
  ...
  function run(event) {
    ...
    if (countdownValue && countdownValue > 0 && Number(n) >= 0 && Number.isInteger(Number(n)) && n !== "" && unit !== "unit" && view !== "select" ) {
        ...
        // Check if musicAudio exists and pause it if it does
        if (musicAudio) {
            musicAudio.pause();
        }
...
submitButton.addEventListener("click", (event) => {
  ...
  function startSimulation(n, set_time, unit, view, color1, color2) {
    ...
    function getRandomColorBetween(color1, color2) {
      if (
        ...
        const randomR =
          Math.floor(Math.random() * (color2.r - color1.r + 1)) + color1.r;
        const randomG =
          Math.floor(Math.random() * (color2.g - color1.g + 1)) + color1.g;
        const randomB =
          Math.floor(Math.random() * (color2.b - color1.b + 1)) + color1.b;
        return `rgb(${randomR}, ${randomG}, ${randomB})`;
      }
    }
    ...
    function createRandomGradientPattern(n) {
      let gradientPattern = `background-color: ${getRandomColorBetween(
        rgbColor1,
        rgbColor2
      )}; background-image: `;
      ...
      for (let i = 0; i < n; i++) {
        ...
        gradientPattern += `radial-gradient(circle at ${randomPositionX}% ${randomPositionY}%, ${getRandomColorBetween(
          rgbColor1,
          rgbColor2
        )} 0%, transparent 50%), `;
      }
    ...
    if (view === "custom") {
      ...
      document.body.style.backgroundColor = getRandomColorBetween(
        rgbColor1,
        rgbColor2
      );
      setInterval(() => {
      ...
      let gradientType =
        view === "conic"
          ...
          ? "linear-gradient"
          : "radial-gradient";
      document.body.style.background = `${gradientType}(${numberColorsBetween(
        rgbColor1,
        rgbColor2,
        n - 1
      )}, ${getRandomColorBetween(rgbColor1, rgbColor2)})`;
      setInterval(() => {
        document.body.style.background = `${gradientType}(${numberColorsBetween(
          rgbColor1,
          rgbColor2,
  ...
  randomizeButton.addEventListener("click", () => {
    ...
    const randomViewIndex =
      Math.floor(Math.random() * (viewSelect.options.length - 1)) + 1;
    viewSelect.selectedIndex = randomViewIndex;

    const soundSelect = document.getElementById("sound");
    const randomSoundIndex = Math.floor(
      Math.random() * soundSelect.options.length
    );
